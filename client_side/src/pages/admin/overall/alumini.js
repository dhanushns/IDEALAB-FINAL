...
function ScheduleVisitTable({ handleScroll }) {
    ...
    useEffect(() => {
        setvisit(JSON.parse(localStorage.getItem('value')).alumini);
    }, []);
            ...
            <div class='db-modal'>
                ...
                <div class='db-content'>
                    <div className='d-flex flex-wrap mt-2 mb-2 justify-content-between'>
                        <div style={{ fontSize: '20px', color: "#fff" }}>Alumini</div>
                    </div>
                                ...
                                <>
                                    <div class='table-content' onScroll={handleScroll}>
                                        <div class='column-header'>
                                            ...
                                            <div class='column-title td c-4'>Programme</div>
                                            <div class='column-title td c-7'>Year of Passing</div>
                                        </div>
                                        {visit.map((item, i) => (
                                            <div class='table-rows' key={i}>
                                                <div class='row-flex' id={`row-flex-${i}`}>
                                                    <div id={`row-wrapper-${i}`} class='data-wrapper'>
                                                        ...
                                                        <div class='row-data rd-6 td text-wrapper'>{item.programme}</div>
                                                        <div class='row-data c-7 td text-wrapper'>{item.yearOfPassing}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
