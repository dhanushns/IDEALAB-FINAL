...
function ScheduleVisitTable({ handleScroll }) {
    ...
    useEffect(() => {
        setvisit(JSON.parse(localStorage.getItem('value')).hostinstitutefaculty);
    }, []);
            ...
            <div class='db-modal'>
                ...
                <div class='db-content'>
                    <div className='d-flex flex-wrap mt-2 mb-2 justify-content-between'>
                        <div style={{ fontSize: '20px', color: "#fff" }}>Host Faculty</div>
                    </div>
                                ...
                                <>
                                    <div class='table-content' onScroll={handleScroll}>
                                        <div class='column-header'>
                                            ...
                                            <div class='column-title td c-4'>Faculty ID</div>
                                            <div class='column-title td c-7'>Designation</div>
                                        </div>
                                        {visit.map((item, i) => (
                                            <div class='table-rows' key={i}>
                                                <div class='row-flex' id={`row-flex-${i}`}>
                                                    <div id={`row-wrapper-${i}`} class='data-wrapper'>
                                                        ...
                                                        <div class='row-data rd-3 td text-wrapper'>{item.FacultyName}</div>
                                                        <div class='row-data rd-4 td text-wrapper'>{item.DateofBirth}</div>
                                                        <div class='row-data rd-5 td text-wrapper'>{item.MobileNumber}</div>
                                                        <div class='row-data rd-6 td text-wrapper'>{item.FacultyID}</div>
                                                        <div class='row-data c-7 td text-wrapper'>{item.Designation}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
