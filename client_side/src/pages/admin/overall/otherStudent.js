...
function ScheduleVisitTable({ handleScroll }) {
    ...
    useEffect(() => {
        setvisit(JSON.parse(localStorage.getItem('value')).otherstudent);
    }, []);
            ...
            <div class='db-modal'>
                ...
                <div class='db-content'>
                    <div className='d-flex flex-wrap mt-2 mb-2 justify-content-between'>
                        <div style={{ fontSize: '20px', color: "#fff" }}>Other Students</div>
                    </div>
                                ...
                                <>
                                    <div class='table-content' onScroll={handleScroll}>
                                        <div class='column-header'>
                                            ...
                                            <div class='column-title td c-4'>Register No</div>
                                            <div class='column-title td c-7'>Year of Studying</div>
                                        </div>
                                        {visit.map((item, i) => (
                                            <div class='table-rows' key={i}>
                                                <div class='row-flex' id={`row-flex-${i}`}>
                                                    <div id={`row-wrapper-${i}`} class='data-wrapper'>
                                                        ...
                                                        <div class='row-data rd-6 td text-wrapper'>{item.studentId}</div>
                                                        <div class='row-data c-7 td text-wrapper'>{item.yearOfStudy}</div>
                                                    </div>
                                                </div>
                                            </div>
